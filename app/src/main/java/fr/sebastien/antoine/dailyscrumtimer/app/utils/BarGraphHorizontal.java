...
public class BarGraphHorizontal extends View {
    ...
    public BarGraphHorizontal(Context context) {
        super(context);
        mContext = context;
    }

    public BarGraphHorizontal(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
    }
    ...
    public void onDraw(Canvas ca) {
        if (mFullImage == null || mShouldUpdate) {
            ...
            for (final Bar bar : mBars) {
                // Set bar bounds
                int left = (int) ((padding * 2) * count + padding + barWidth * count - bottomPadding);
                int right = (int) (getHeight()  - (usableHeight * (bar.getValue() / maxValue)));
                int top  = (int) ((padding * 2) * count + padding + barWidth * (count + 1) - bottomPadding);
                int bottom = (int) (getHeight());
